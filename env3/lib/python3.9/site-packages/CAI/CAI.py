...
def _synonymous_codons(genetic_code_dict):
    ...
    for codon, amino_acid in list(genetic_code_dict.items()):
    ...
    return {codon: codons_for_amino_acid[genetic_code_dict[codon]] for codon in list(genetic_code_dict.keys())}


_synonymous_codons = {k: _synonymous_codons(v.forward_table) for k, v in list(ct.unambiguous_dna_by_id.items())}
_non_synonymous_codons = {k: {codon for codon in list(v.keys()) if len(v[codon]) == 1} for k, v in list(_synonymous_codons.items())}
